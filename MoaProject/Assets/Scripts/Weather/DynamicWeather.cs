...
public class DynamicWeather : MonoBehaviour {
    ...
    public ParticleSystem ThunderStormParticleSystem;
    ...
    private ParticleSystem.EmissionModule ThunderStormEmission;
	...
	void Start () {
        ...
        ThunderStormEmission = ThunderStormParticleSystem.emission;
    ...
    void PickWeatherState()
    {
        ...
        ThunderStormEmission.enabled = false;
        ...
        switch (_SwitchWeather)
        {
            ...
            case 3:
                ...
                break;
            case 4:
                CurrentWeatherState = DynamicWeather.WeatherStates.Snow;
                break;
        }
    ...
    {
        Debug.Log("Thunder");
        ThunderStormEmission.enabled = true;
    ...
    void Snow()
    {
        ...
        //if light intensity is greater than SnowIntensity, decrease by time.deltatime * LightDimTime
        if (GetComponent<Light>().intensity > SnowIntensity)
        {
            GetComponent<Light>().intensity -= Time.deltaTime * LightDimTime;
        }
        //if light intensity is less than SnowIntensity, increase by time.deltatime * LightDimTime
        if (GetComponent<Light>().intensity < SnowIntensity)
        {
            GetComponent<Light>().intensity += Time.deltaTime * LightDimTime;
        }
        ...
        //if volume is greater than 0 and audio doesn't equal current weather audio, start reducing volume by AudioFadeTime
        if (GetComponent<AudioSource>().volume > 0 && GetComponent<AudioSource>().clip != SnowAudio)
        {
            GetComponent<AudioSource>().volume -= Time.deltaTime * AudioFadeTime;
        }
        ...
        {
            GetComponent<AudioSource>().Stop(); //stop playing audio
            GetComponent<AudioSource>().clip = SnowAudio;//set audio to snow
            GetComponent<AudioSource>().loop = true; //set to loop
            GetComponent<AudioSource>().Play(); //start playing
        }

        //if volume is less than 1 and audio equals current weather audio, start increasing volume by AudioFadeTime
        if (GetComponent<AudioSource>().volume < 1 && GetComponent<AudioSource>().clip == SnowAudio)
        {
            GetComponent<AudioSource>().volume += Time.deltaTime * AudioFadeTime;
        }
    }
