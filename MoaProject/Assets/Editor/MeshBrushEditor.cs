...
namespace MeshBrush
{
    ...
    public class MeshBrushEditor : Editor
    {
        ...
        Vector3 lastPaintLocation = Vector3.zero;       // The last paint location (used for brush stroke alignment).
        Vector3 brushStrokeDirection = Vector3.zero;    // The direction of the brush stroke in relation to the last paint location.

        int globalPaintLayerMask = 1;

        int totalPaintedMeshes = 0;
        int totalPaintedTris = 0;
        ...
        void OnEnable()
        {
            ...
            _mbp = _mb.holderObj.GetComponent<MeshBrushParent>();
        ...
        public override void OnInspectorGUI() // Works like OnGUI, except that it updates only the inspector view.
        {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                EditorGUILayout.LabelField(new GUIContent("Enabled", "Enables/disables this MeshBrush group."), GUILayout.Width(60f), GUILayout.ExpandWidth(false));

                // Triangle counter label toggle
                _mbs.trisCounter = EditorGUILayout.Toggle(_mbs.trisCounter, GUILayout.Width(15f));
                EditorGUILayout.LabelField(new GUIContent("Tris-Counter", "Displays a tris-counter label at the top of the MeshBrush inspector.\n\nThe displayed numbers refer to the active, selected MeshBrush group (its painted meshes)."), GUILayout.Width(84f), GUILayout.ExpandWidth(false));

                if (_mbs.lockSceneView)
                {
                    GUI.color = Color.yellow;
                }
                _mbs.lockSceneView = EditorGUILayout.Toggle(_mbs.lockSceneView, GUILayout.Width(15f));
                EditorGUILayout.LabelField(new GUIContent("Lock scene view", "Avoids losing focus of the scene view or accidentally deselect this GameObject when painting and clicking around on other objects in the scene.\n\nThis is particularly useful if you're painting with your mouse buttons as paint keys or even tablets."), GUILayout.Width(100f), GUILayout.ExpandWidth(false));
                if (_mbs.lockSceneView)
                {
                    GUI.color = Color.white;
                }
            ...
            if (_mbs.b_BrushSettings)
            {
                ...
                _mbs.delay = EditorGUILayout.Slider(new GUIContent("Delay [s]:", "If you press and hold down the paint button, this value will define the delay (in seconds) between paint strokes; thus, the higher you set this value, the slower you'll be painting meshes."), _mbs.delay, 0.03f, 1.0f);
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    ...
                    _mbs.alignWithStroke = EditorGUILayout.Toggle(_mbs.alignWithStroke, GUILayout.Width(15f));
                ...
                GUI.enabled = lastPaintLocation != Vector3.zero && brushStrokeDirection != Vector3.zero && _mbs.alignWithStroke;
                ...
                {
                    lastPaintLocation = Vector3.zero;
            ...
            if (_mbs.b_Opt)
            {
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    if (GUILayout.Button(new GUIContent("Flag all painted\nmeshes as static", "Flags all the meshes you've painted so far as static in the editor.\nCheck out the Unity documentation about drawcall batching if you don't know what this is good for."), GUILayout.Height(50f), GUILayout.Width(150f), GUILayout.ExpandWidth(true)) && _mbp)
                        _mbp.FlagMeshesAsStatic();
                    if (GUILayout.Button("Unflag all painted\nmeshes as static", GUILayout.Height(50f), GUILayout.Width(150f), GUILayout.ExpandWidth(true)) && _mbp)
                        _mbp.UnflagMeshesAsStatic();
                }
                EditorGUILayout.EndHorizontal();
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    if (GUILayout.Button(new GUIContent("Combine all painted meshes", "Once you're done painting meshes, you can click here to combine them all. This will combine all the meshes you've painted into one single mesh (one per material).\n\nVery useful for performance optimization.\nCannot be undone."), GUILayout.Height(50f), GUILayout.Width(150f), GUILayout.ExpandWidth(true)))
                    {
                        if (_mb.holderObj != null)
                        {
                            _mbp.CombinePaintedMeshes(_mbs.autoSelectOnCombine, _mb.holderObj.GetComponentsInChildren<MeshFilter>());
                            UpdateTrisCounter();
                        }
                    }

                    //...and one to delete all the meshes we painted on this GameObject so far.
                    if (GUILayout.Button(new GUIContent("Delete all painted meshes", "Are you sure? This will delete all the meshes you've painted onto this GameObject's surface so far (except already combined meshes)."), GUILayout.Height(50f), GUILayout.Width(150f), GUILayout.ExpandWidth(true)) && _mbp)
                    {
                        ClearDeletionBuffer();
                        ...
                        for (int i = 0; i < _mb.setOfMeshesToPaint.Count; i++)
                        {
                            ...
                            switch (currentEvent.type)
                            {
                                case EventType.MouseDown:
                                    ...
                                    if (dropAreaMesh.Contains(currentEvent.mousePosition))
                                    {
                                        ...
                                        else if (currentEvent.button == 1 && _mb.setOfMeshesToPaint.Count > 1) // Right mouse button for entry deletion.
                                        {
                                            Undo.RecordObject(_mb, "object field removal (MeshBrush)");
                                            _mb.setOfMeshesToPaint.RemoveAt(i);
                                        }
                                        else if (currentEvent.button == 2) // Press the mouse wheel on an object entry to nullify it.
                                        {
                                            Undo.RecordObject(_mb, "nullification (MeshBrush)");
                                            _mb.setOfMeshesToPaint[i] = null;
                                        }
                                ...
                                case EventType.DragPerform:
                                    ...
                                    if (!dropAreaMesh.Contains(currentEvent.mousePosition) || DragAndDrop.objectReferences.Length > 1)
                                        break;
        ...
        void OnSceneGUI() // http://docs.unity3d.com/Documentation/ScriptReference/Editor.OnSceneGUI.html
        {
            ...
            if (_mbs.isActive)
            {
                ...
                if (!_mb.globalPaintingMode && thisCollider == null)
                    return;
                ...
                // Draw the triangle counter label. 
                /* [EDIT: now in form of a label on top of the MeshBrush inspector]
                if (_mbs.trisCounter)
                {
                    Handles.BeginGUI();
                    {
                        GUILayout.Window(2, new Rect(Screen.width - 180, Screen.height - 80, 170, 50), (id) => { GUILayout.Label("Meshes: " + totalPaintedMeshes + "\n" + "Triangles: " + totalPaintedTris); }, "Tris-Counter");
                    }
                    Handles.EndGUI();
                }
                */
                ...
                if (currentEvent.keyCode == _mbs.increaseRadiusKey)
                    _mbs.brushRadius += 0.05f;
                else if (currentEvent.keyCode == _mbs.decreaseRadiusKey && _mbs.brushRadius > 0)
                    _mbs.brushRadius -= 0.05f;
                ...
                if (_mb.globalPaintingMode ? Physics.Raycast(scRay, out scHit, Mathf.Infinity, globalPaintLayerMask) : thisCollider.Raycast(scRay, out scHit, Mathf.Infinity))
                {
                    ...
                    if (canPaint)
                    {
                        if(currentEvent.type == EventType.MouseDown)
                        {
                            ...
                            if (_mbs.paintKey == KeyCode.Mouse0 && currentEvent.button == 0)
                                Paint();
                            else if (_mbs.deleteKey == KeyCode.Mouse0 && currentEvent.button == 0)
                                Delete();
                            else if (_mbs.combineAreaKey == KeyCode.Mouse0 && currentEvent.button == 0)
                        ...
                        else if (currentEvent.type == EventType.KeyDown)
                        {
                            ...
                            if (currentEvent.keyCode == _mbs.paintKey)
                                Paint();
                            else if (currentEvent.keyCode == _mbs.deleteKey)
                                Delete();
                            else if (currentEvent.keyCode == _mbs.combineAreaKey)
                                CombineMeshesInBrushArea();
                        }
                    }
                }
        ...
        void BrushMode_PrecisionPlacement() // Mode for the precise placement of single meshes.
        {
            ...
            for (int i = 0; i < _mb.setOfMeshesToPaint.Count; i++)
            {
                ...
                if (_mb.setOfMeshesToPaint[i] == null || (currentEvent.type == EventType.KeyDown && currentEvent.keyCode == KeyCode.Escape))
                {
                    if (_mb.setOfMeshesToPaint[i] == null)
            ...
            if (Selection.gameObjects.Length == 1 && Selection.activeGameObject.transform == thisTransform)
            {
                ...
                scRay = HandleUtility.GUIPointToWorldRay(currentEvent.mousePosition);
                ...
                switch (_phase)
                {
                    case 0:
                        ...
                        {
                            previewObj.transform.position = scHit.point;
                            previewObj.transform.up = Vector3.Lerp(Vector3.up, scHit.normal, _mbs.slopeInfluence * 0.01f);
                        ...
                        if (currentEvent.type == EventType.KeyDown && currentEvent.keyCode == _mbs.paintKey ||( currentEvent.type == EventType.MouseDown && currentEvent.button == 0 && _mbs.paintKey == KeyCode.Mouse0))
                        {
        ...
        void Paint() // The actual paint function.
        {
            ...
            else
            {
                ...
                brushStrokeDirection = scHit.point - lastPaintLocation;
        ...
        void PaintSingleMesh() // Single meshpaint function.
        {
            ...
            if ((_mbs.inverseSlopeFilter == true) ? (slopeAngle > _mbs.maxSlopeFilterAngle) : (slopeAngle < _mbs.maxSlopeFilterAngle))
            {
                ...
                Vector3 newUp = Vector3.Lerp(_mbs.yAxisIsTangent ? paintedMeshTransform.up : Vector3.up, paintedMeshTransform.forward, _mbs.slopeInfluence * 0.01f);

                // Brush stroke alignment
                Vector3 newFwd = _mbs.alignWithStroke && (brushStrokeDirection != Vector3.zero && lastPaintLocation != Vector3.zero) ? brushStrokeDirection : paintedMeshTransform.forward;

                // We need to orthonormalize the two vectors before applying the rotation.
                Vector3.OrthoNormalize(ref newUp, ref newFwd);

                paintedMeshTransform.rotation = Quaternion.LookRotation(newFwd, newUp);
        ...
        void CombineMeshesInBrushArea()
        {
            ...
            if (meshFilters != null && meshFilters.Length > 0)
            {
                ...
                if (meshFiltersToCombine.Count > 0)
                    _mbp.CombinePaintedMeshes(_mbs.autoSelectOnCombine, meshFiltersToCombine.ToArray());
            }
        ...
        void UpdateTrisCounter()
        {
            ...
            totalPaintedMeshes = _mbp.GetMeshCount();
            totalPaintedTris = _mbp.GetTrisCount();
        }
        #endregion
